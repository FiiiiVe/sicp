...
(define (make-connector)
  (let ((value false) (informant false) (constraints '()))
    (define (set-my-value new-value)
      (if (not (equal? value new-value))
          (begin (set! value new-value)
                 (for-each-except informant inform-me)
                 (for-each-except informant propagate))
          'done))
    ...
    (define (connect new-constraint)
      (if (not (memq new-constraint constraints))
          ...
          'done)
      (new-constraint value informant))
    (define (inform-me)
      (if informant
          (informant value)
          'done ))
    (define (propagate)
      (for-each (lambda (c) (c value))
                constraints))
    (define (me m)
      (cond ((eq? m 'has-value?) (if informant true false))
            ((eq? m 'value) value)
            ((eq? m 'set-value!) set-my-value)
            ((eq? m 'forget) forget-my-value)
...
(define (probe name connector)
  ...
  (define (process-new-value)
    (print-probe (connector 'get-value)))
...
(define (stream-display s count)
  (if (or (= count 0) (null?  s))
      (begin
        (display "done")
        (newline))
      (begin
        ...
        (display ", ")
        (stream-display (stream-cdr s) (- count 1)))))
(define (stream-map proc . argstreams)
  (if (null? (car argstreams))
      '()
      (cons-stream
       (apply proc (map (lambda (x) (stream-car x))
                        argstreams))
       (apply stream-map
